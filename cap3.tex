...
\subsection{Distribución y Sincronización de Dominios de Reloj}
\label{ssec:clock_wizard}
A diferencia de un esquema centralizado en el que un único IP de tipo
\textit{Clocking Wizard} (MMCM/PLL) sintetiza todos los relojes del
sistema a partir de un oscilador externo, la arquitectura final no
instancia ningún bloque de acondicionamiento de reloj en la lógica
programable: los dos relojes utilizados, de 50MHz y 100MHz, se obtienen
directamente de las salidas \texttt{FCLK\_CLK1} y \texttt{FCLK\_CLK0} del
bloque \texttt{processing\_system7\_0}, generadas internamente por el
Procesador de Sistema (PS7) y configuradas mediante los parámetros
\texttt{PCW\_FPGA1\_PERIPHERAL\_FREQMHZ=50} y
\texttt{PCW\_FPGA0\_PERIPHERAL\_FREQMHZ=100} respectivamente. El reloj de
50MHz (\texttt{FCLK\_CLK1}) alimenta exclusivamente al subsistema
Ibex+BRAM (\texttt{ibex\_bram\_wrapper\_bd\_0/clk\_i}), mientras que el
reloj de 100MHz (\texttt{FCLK\_CLK0}) alimenta exclusivamente al
periférico Reporter (\texttt{axi\_reporter\_0/s00\_axi\_aclk}). El reinicio
(\textit{reset}) del núcleo Ibex, por su parte, no depende de ninguna
señal de \textit{locked} de un PLL -- inexistente en este diseño -- sino
que es controlado explícitamente por software a través de un canal AXI
GPIO (\texttt{axi\_gpio\_0}), permitiendo a Linux mantener el núcleo en
reposo hasta el instante deseado para iniciar una nueva corrida
experimental. El cruce de información entre ambos dominios de reloj no se
resuelve mediante colas de aislamiento (FIFOs asíncronas), sino mediante
un esquema de dos etapas complementarias: del lado del Ibex, un par de
registros \textit{sticky} (\texttt{reporter\_addr\_hold} y
\texttt{reporter\_wdata\_hold}) retienen la dirección y el dato de la
última escritura real hasta la siguiente, garantizando que permanezcan
estables el tiempo suficiente para ser observados de forma segura desde
el otro dominio; del lado del Reporter, una cadena sincronizadora de tres
\textit{flip-flops} resuelve el pulso de escritura entrante, exponiendo
además, con fines de diagnóstico, un camino paralelo de un único
\textit{flip-flop} sin sincronizar (véanse los registros
\texttt{DBG\_RAW\_COUNT} y \texttt{DBG\_SYNC\_COUNT} de la
Tabla~\ref{tab:mapa_registros_reporter}) que permite contrastar, en
operación normal, que ningún evento se pierda por metaestabilidad.
...
La memoria de bloque (\textit{Block RAM}) que almacena las instrucciones
del núcleo Ibex se generó a partir de la primitiva \texttt{blk\_mem\_gen}
v8.4 de Xilinx, configurada con registro de salida habilitado en ambos
puertos (\texttt{C\_HAS\_MEM\_OUTPUT\_REGS\_A} y
\texttt{C\_HAS\_MEM\_OUTPUT\_REGS\_B} en 1). Esta configuración, elegida
para maximizar la frecuencia máxima alcanzable del diseño, introduce una
latencia real de \textbf{dos ciclos de reloj} entre la presentación de una
dirección válida y la disponibilidad del dato correspondiente a la
salida, en contraposición a la latencia de un único ciclo con la que se
había modelado inicialmente la señal de válido de lectura
(\texttt{instr\_rvalid\_q}/\texttt{data\_rvalid\_q}) en
\texttt{ibex\_bram\_subsystem.sv}. Este desajuste de un ciclo resulta
invisible durante una secuencia de \textit{fetch} puramente secuencial,
donde tanto la dirección como el dato avanzan de a uno por ciclo sin
huecos, pero se vuelve crítico inmediatamente después de cualquier salto o
bifurcación tomada: el primer \textit{fetch} posterior a la redirección
del contador de programa termina capturando el dato correspondiente a la
solicitud \textit{anterior}, todavía en tránsito dentro de la memoria, en
lugar del correspondiente a la nueva dirección. La corrección consistió en
modelar la señal de válido mediante una cadena de dos \textit{flip-flops}
en cascada, en lugar de uno solo, replicando fielmente la latencia real de
la primitiva de memoria instanciada.
...
Además, la herramienta de síntesis fue configurada para aceptar un archivo de inicialización de coeficientes (\texttt{.coe}), el cual permite que la memoria BRAM se instancie con el firmware del Ibex
    pre-cargado desde el momento exacto en que se programa la FPGA (\textit{bitstream}), eliminando la necesidad de que el ARM transfiera el código de arranque rutinariamente. El proceso completo
    de compilación, desde el código fuente en C hasta este archivo \texttt{.coe} -- incluyendo la instalación del \textit{toolchain} y el código fuente completo de los firmwares utilizados -- se
    documenta en detalle en el Apéndice~\ref{ap:pipeline_firmware}.
...
\begin{itemize}
    ...
    \item \textbf{Profundidad de Celdas (\textit{Memory Depth}):} Se fijó una profundidad de \textbf{$4096$ celdas} (posiciones de memoria discretas).
     \item \textbf{Capacidad Total de Almacenamiento:} El producto de las dimensiones físicas ($4096 \text{ posiciones} \times 32 \text{ bits}$) equivale a un total de $131{,}072\text{ bits}$ de memoria interna. En términos de almacenamiento direccionable por bytes, esto representa una capacidad neta de \textbf{$16\text{ KiB}$}, si bien la ventana de acceso expuesta a Linux a través del \textit{AXI BRAM Controller} se limita a los primeros $8\text{ KiB}$ (ver Tabla~\ref{tab:mapa_memoria}).
...
\begin{tabular}{llcl}
\hline
\textbf{Periférico / Interfaz} & \textbf{Master base Address}	& \textbf{Range}	& \textbf{Acceso desde Ibex}				\\ \hline
...
HR-RT-Reporter (IP Core)       & \texttt{0x43C00000}		& $64\text{ KB}$	& \texttt{0x80000008}--\texttt{0x80000010} (bloque de captura de eventos, ver Tabla~\ref{tab:mapa_registros_reporter})         \\ \hline
\end{tabular}
\end{table}
...
De igual manera, el módulo de trazabilidad por hardware es accedido
    por el ARM para la extracción asíncrona de marcas de tiempo en la
    dirección física \texttt{0x43C00000}, mientras que el Ibex escribe
    directamente en las tres direcciones reservadas dentro de ese mismo
    rango (\texttt{0x80000008}, \texttt{0x8000000C} y \texttt{0x80000010},
    ver Sección~\ref{sec:axi_reporter}) para activar los disparadores de
    captura sin provocar retrasos por contención de buses.
...
Este contador es alimentado directamente por el reloj de
$100\text{ MHz}$ propio del periférico Reporter (\texttt{FCLK\_CLK0} del Procesador de Sistema), operando de forma independiente y asíncrona respecto del reloj de $50\text{ MHz}$ del núcleo Ibex. Como consecuencia, el registro temporal se incrementa de forma inexorable cada ciclo de reloj de $100\text{ MHz}$, proveyendo una base de tiempo monótona con una resolución discreta y absoluta de $10\text{ ns}$.
...
\begin{enumerate}
    \item \textbf{Decodificación de Dirección de Frontera:} Dentro del
  firmware en \texttt{.coe} ejecutado por el Ibex, cada vez que el
  algoritmo alcanza un punto de control crítico o un evento de interés, el
  compilador emite una instrucción estándar de almacenamiento
  (\textit{Store Word}, \texttt{sw}) apuntando a una de tres direcciones de
  memoria virtual reservadas dentro del rango del periférico:
  \texttt{0x80000008} para el contador de programa del evento
  (\texttt{REG\_PC}), \texttt{0x8000000C} para su índice
  (\texttt{REG\_I}) y \texttt{0x80000010} para el dato asociado
  (\texttt{REG\_D}). El decodificador de direcciones del subsistema
  (señal \texttt{sel\_ts} en \texttt{ibex\_bram\_subsystem.sv}) desvía
  hacia el Reporter cualquier acceso dentro del rango completo
  \texttt{0x8000\_0000}--\texttt{0x8FFF\_FFFF}; dentro de ese rango, es el
  propio núcleo de captura del Reporter (\texttt{reporter\_core.sv}) quien
  distingue el tipo de evento comparando la dirección exacta de cada
  escritura entrante, en lugar de tratar la totalidad del rango como un
  único registro pasivo indiferenciado.
    \item \textbf{Sincronización entre Dominios de Reloj (CDC):} Dado que
    el núcleo Ibex opera a $50\text{ MHz}$ y el periférico Reporter a
    $100\text{ MHz}$ -- dos dominios de reloj asíncronos entre sí, ver
    Sección~\ref{ssec:clock_wizard} -- el pulso de escritura no puede
    muestrearse de forma combinacional sin riesgo de metaestabilidad. Del
    lado del Ibex, un par de registros \textit{sticky} retienen la
    dirección y el dato de la última escritura hasta la siguiente; del
    lado del Reporter, una cadena sincronizadora de tres
    \textit{flip-flops} resuelve de forma segura el pulso de escritura
    entrante antes de disparar la captura del evento.
    \item \textbf{Registro del Evento:} Una vez resuelto el pulso de
    escritura por el sincronizador -- con una latencia de dos a tres
    ciclos del reloj de $100\text{ MHz}$, es decir entre $20$ y
    $30\text{ ns}$ -- el \texttt{axi\_reporter} captura y almacena en sus
    registros internos \textit{sticky} el valor exacto del contador de
    $64\text{ bits}$ en ese instante, junto con el dato o índice de evento
    retenido por los registros \textit{sticky} del lado del Ibex.
\end{enumerate}
...
\subsubsection{Mapa de Registros del Reporter}
El periférico \texttt{axi\_reporter\_v1.0} expone dos interfaces de
acceso completamente independientes entre sí, correspondientes a los dos
maestros que interactúan con él. Desde el lado del núcleo Ibex, el acceso
es de \textbf{solo escritura} y no pasa por ningún decodificador de
registros: el firmware emite instrucciones \texttt{sw} directamente hacia
tres direcciones absolutas dentro del espacio de memoria del Ibex
(\texttt{0x80000008} para el contador de programa del evento,
\texttt{0x8000000C} para su índice, y \texttt{0x80000010} para el dato
asociado), y es el propio núcleo de captura (\texttt{reporter\_core.sv})
quien distingue el tipo de evento comparando la dirección exacta de cada
escritura entrante. Desde el lado del bus AXI-Lite -- por el cual el
Procesador de Sistema (PS) y, en consecuencia, el sistema operativo Linux
embebido acceden al periférico -- el Reporter se comporta como un banco
convencional de \textbf{ocho registros de solo lectura} de 32 bits,
mapeados en la dirección base \texttt{0x43C00000} y direccionados por un
desplazamiento (\textit{offset}) de 0x00 a 0x1C, tal como se detalla en la
Tabla~\ref{tab:mapa_registros_reporter}.

Cada registro cumple un rol específico dentro del esquema de
trazabilidad. Los dos primeros (\texttt{TIMESTAMP\_LO}/\texttt{HI})
forman un contador libre de 64 bits que corre de manera continua en el
dominio de reloj de 100MHz del propio Reporter, independiente de la
actividad del Ibex. Los siguientes tres (\texttt{EVT\_PC},
\texttt{EVT\_I}, \texttt{EVT\_D}) son registros de tipo \textit{sticky}:
retienen el último valor escrito por el Ibex hasta que ocurra una nueva
escritura sobre esa misma dirección, de forma que Linux siempre puede leer
el estado más reciente del evento sin necesidad de sincronizarse con el
instante exacto en que el Ibex escribió. El registro \texttt{EVENT\_VALID}
es, en cambio, un pulso de un único ciclo de reloj (a 100MHz) que se activa
en el instante en que se completa la sincronización de una escritura
proveniente del dominio de 50MHz del Ibex; dada su corta duración, es
poco probable que un sondeo por software lo capture en el instante exacto,
por lo que su utilidad es más de depuración puntual que de monitoreo
continuo. Finalmente, \texttt{DBG\_RAW\_COUNT} y \texttt{DBG\_SYNC\_COUNT}
son contadores de diagnóstico del propio mecanismo de sincronización entre
dominios de reloj (ver Sección~\ref{sec:cdc}): el primero cuenta flancos
de escritura detectados sin pasar por el sincronizador (una vía
deliberadamente insegura, expuesta solo para comparación), mientras que el
segundo cuenta los flancos ya sincronizados de forma segura; en operación
normal ambos deben coincidir, y cualquier divergencia entre ellos indicaría
pérdida de eventos por metaestabilidad en el cruce de dominios de reloj.

\begin{table}[htbp]
\centering
\caption{Mapa de registros del periférico \texttt{axi\_reporter\_v1.0}, accesibles por AXI-Lite desde el lado del Procesador de Sistema (offset relativo a la dirección base \texttt{0x43C00000}).}
\label{tab:mapa_registros_reporter}
\begin{tabular}{clcl}
\hline
\textbf{Offset} & \textbf{Registro} & \textbf{Acceso} & \textbf{Descripción} \\ \hline
\texttt{0x00} & \texttt{TIMESTAMP\_LO}   & Solo lectura & Bits [31:0] del contador libre de 64 bits (100MHz) \\
\texttt{0x04} & \texttt{TIMESTAMP\_HI}   & Solo lectura & Bits [63:32] del contador libre de 64 bits \\
\texttt{0x08} & \texttt{EVT\_PC}         & Solo lectura & Último contador de programa escrito por el Ibex (\textit{sticky}) \\
\texttt{0x0C} & \texttt{EVENT\_VALID}    & Solo lectura & Pulso de 1 ciclo (100MHz) al completarse la sincronización de un evento \\
\texttt{0x10} & \texttt{EVT\_D}          & Solo lectura & Último dato de evento escrito por el Ibex (\textit{sticky}) \\
\texttt{0x14} & \texttt{EVT\_I}          & Solo lectura & Último índice de iteración escrito por el Ibex (\textit{sticky}) \\
\texttt{0x18} & \texttt{DBG\_RAW\_COUNT} & Solo lectura & Cuenta de flancos de escritura \emph{sin} sincronizar (diagnóstico de CDC) \\
\texttt{0x1C} & \texttt{DBG\_SYNC\_COUNT}& Solo lectura & Cuenta de flancos de escritura ya sincronizados (la cuenta confiable) \\ \hline
\end{tabular}
\end{table}

Estos 8 \textit{offsets} se leen desde el lado AXI\-Lite/Linux (base
    \texttt{0x43C00000}), y no deben confundirse con las tres direcciones
    de escritura que usa el firmware del lado del Ibex
    (\texttt{0x80000008}/\texttt{0x8000000C}/\texttt{0x80000010}, ver
    Tabla~\ref{tab:mapa_memoria}) -- son dos espacios de direcciones
    distintos para el mismo periférico.
...
La comunicación se centraliza a través del bloque IP \textit{AXI
    Interconnect}, configurado para expandir el puerto maestro de
    propósito general del ARM (\texttt{M\_AXI\_GP0}) hacia cuatro puertos
    esclavos independientes M-AXI4-Lite operando a una frecuencia de bus
    de $100\text{ MHz}$. Las direcciones físicas de mapeo para los
    registros de control y datos se estructuraron como se muestra en la
    tabla \ref{tab:mapa_memoria}. El detalle completo de las conexiones
    punto a punto autogeneradas por Vivado para cada bloque IP del
    diseño se incluye en el Apéndice~\ref{ap:conexiones}.
...
\subsection{Scripts de Validación primeros ensayos}
...
En el capítulo \ref{ch:resultados} se implementan scripts específicos destinados a abordar la hipótesis principal y a resolver los problemas identificados durante el desarrollo de estos primeros ensayos.
\end{enumerate}
