...
Para cumplir con la precisión que exige un Sistema de Tiempo Real
     Estricto (\textit{Hard Real-Time}), la medición no puede depender de
     contadores gestionados por el sistema operativo, dadas las latencias
     variables de interrupción. Por ello, la generación de marcas
     temporales (\textit{timestamps}) se implementa mediante un circuito
     digital dedicado en la Lógica Programable (PL), alimentado por un
     reloj de $100\text{ MHz}$ generado internamente por el Procesador de
     Sistema (PS7) y exclusivo del periférico Reporter -- independiente
     del reloj de $50\text{ MHz}$ que utiliza el núcleo Ibex. A partir de
     esta señal se instancia un contador de giro libre en hardware puro de
     $64\text{ bits}$, que se incrementa cada $10\text{ ns}$.

     La implementación del sistema sobre la arquitectura heterogénea Zynq
     presenta un desafío clásico de diseño digital: la coexistencia de
     dominios de reloj asíncronos. En este caso puntual, el desafío no se
     encuentra entre el procesador ARM (PS) y la Lógica Programable (PL)
     en general -- ese cruce ya lo resuelve de forma transparente la
     infraestructura AXI del fabricante -- sino específicamente entre los
     dos relojes que conviven dentro de la propia PL: el núcleo Ibex,
     operando a $50\text{ MHz}$, y el periférico Reporter, operando a
     $100\text{ MHz}$, sin relación de fase fija entre ambos. Transmitir
     señales de un dominio al otro sin las debidas precauciones es
     propenso a generar metaestabilidad y pérdida de eventos, problema que
     se resuelve mediante el esquema de registros \textit{sticky} más
     sincronizador de tres \textit{flip-flops} detallado en la
     Sección~\ref{ssec:clock_wizard} del Capítulo~\ref{ch:metodologia}.
